...
fn read_line<'a>(
    ...
    read_pairs_cb(iter, |code, val| match code {
        10 => x1 = fast_parse_f32(val),
        20 => y1 = fast_parse_f32(val),
        11 => x2 = fast_parse_f32(val),
        21 => y2 = fast_parse_f32(val),
        _ => {}
    });

    Some(DxfEntity::Line { x1, y1, x2, y2 })
}
...
fn read_circle<'a>(
    ...
    read_pairs_cb(iter, |code, val| match code {
        10 => cx = fast_parse_f32(val),
        20 => cy = fast_parse_f32(val),
        40 => r = fast_parse_f32(val),
        _ => {}
    });

    Some(DxfEntity::Circle { cx, cy, r })
}
...
fn read_arc<'a>(
    ...
    read_pairs_cb(iter, |code, val| match code {
        ...
        20 => cy = fast_parse_f32(val),
        40 => r = fast_parse_f32(val),
        50 => start_angle = fast_parse_f32(val),
        51 => end_angle = fast_parse_f32(val),
        _ => {}
    });
...
fn read_polyline<'a>(
    ...
    read_pairs_cb(iter, |code, val| match code {
        10 => x_acc.push(fast_parse_f32(val)),
        20 => y_acc.push(fast_parse_f32(val)),
        _ => {}
    });
...
#[inline(always)]
fn read_pairs_cb<'a, F>(iter: &mut std::iter::Peekable<impl Iterator<Item = &'a str>>, mut cb: F)
where
    F: FnMut(i32, &'a str),
{
    while let Some(peeked) = iter.peek().copied() {
        let peeked = peeked.trim();
        let code: i32 = peeked.parse().unwrap_or(-1);
        if code == 0 {
            break;
        }
        iter.next(); // consume the peeked code
        if let Some(val_line) = iter.next() {
            cb(code, val_line.trim());
        } else {
            break;
        }
    }
