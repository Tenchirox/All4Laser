            ...
            match wctx.wizard.step {
                0 => {
                    ...
                    for lang in &[
                        ...
                        if ui.selectable_label(*wctx.language == *lang, lang.name()).clicked() {
                            *wctx.language = *lang;
                            crate::i18n::set_language(*wctx.language);
                        }
                    }
                    ui.add_space(8.0);
                    if ui.button("Next →").clicked() { wctx.wizard.step = 1; }
                }
                1 => {
                    ...
                    ui.horizontal(|ui| {
                        ui.label("Width (mm):");
                        ui.add(egui::DragValue::new(&mut wctx.machine_profile.workspace_x_mm).speed(10.0));
                    });
                    ui.horizontal(|ui| {
                        ui.label("Height (mm):");
                        ui.add(egui::DragValue::new(&mut wctx.machine_profile.workspace_y_mm).speed(10.0));
                    });
                    ui.add_space(8.0);
                    ui.horizontal(|ui| {
                        if ui.button("← Back").clicked() { wctx.wizard.step = 0; }
                        if ui.button("Next →").clicked() { wctx.wizard.step = 2; }
                    });
                }
                _ => {
                    ...
                    ui.selectable_value(&mut wctx.machine_profile.controller_kind, ControllerKind::Grbl, "GRBL");
                    ui.selectable_value(&mut wctx.machine_profile.controller_kind, ControllerKind::Ruida, "Ruida (beta)");
                    ui.selectable_value(&mut wctx.machine_profile.controller_kind, ControllerKind::Trocen, "Trocen (beta)");
                    if wctx.machine_profile.controller_kind != prev_kind {
                        result.controller_changed = true;
                    }
                    ui.add_space(8.0);
                    ui.horizontal(|ui| {
                        if ui.button("← Back").clicked() { wctx.wizard.step = 1; }
                        if ui.button("✅ Finish").clicked() { result.finished = true; }
                    });
                }
            }
            ui.add_space(4.0);
            if ui.small_button("Skip wizard").clicked() { result.finished = true; }
        });
